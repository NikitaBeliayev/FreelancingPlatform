...
{
    public class GetAllObjectivesWithPaginationQueryHandler : IQueryHandler<GetAllObjectivesWithPaginationQuery, PaginationModel<ResponseObjectiveDto>>
    {	
		...
		public async Task<Result<PaginationModel<ResponseObjectiveDto>>> Handle(GetAllObjectivesWithPaginationQuery request, CancellationToken cancellationToken)
		{
			_logger.LogInformation("Get all objective types with pagination has been requested");
			var (objectives, total) = await _repository.GetAllForImplementorWithPagination(request.PageSize, (request.PageNum - 1) * request.PageSize, cancellationToken);
			if (!objectives.Any())
			{
				return ResponseHelper.LogAndReturnError<PaginationModel<ResponseObjectiveDto>>("No objectives found", new Error("Objective GetObjectives.GetAllWithPagiation.GetAllObjectivesWithPaginationQueryHandler", "No objectives found", 422));
			}
